...
void CCMainWindow::initContacTree()
{
	...
	QSqlQuery querySelfDepID(QString("SELECT departmentID FROM tab_employee WHERE employeeID = %1").arg(gLoginEmployeeID));
...
{
	// 从本地编码，转变成Unicode
	setUserName(QString::fromLocal8Bit("三艺科技"));
...
QString CCMainWindow::getHeadPicPath()
{
	...
	{
		QSqlQuery queryPic(QString("SELECT picture FROM tab_employee WHERE employeeID = %1")
