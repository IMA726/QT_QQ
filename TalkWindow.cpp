...
void TalkWindow::initTalkWindow()
{
	...
	*/
	...
	{	// 如果不是公司群，通过公司群号，进行过滤
		queryEmployeeModel.setQuery(QString("SELECT employeeID FROM tab_employee WHERE status = 1 AND departmentID = %1")
			.arg(m_talkId));
	}
...
void TalkWindow::addPeopInfo(QTreeWidgetItem* pRootGroupItem, int employeeID)
{
	...
	queryInfoModel.setQuery(QString("SELECT employee_name,employee_sign,picture FROM tab_employee WHERE employeeID = %1")
		.arg(employeeID));
