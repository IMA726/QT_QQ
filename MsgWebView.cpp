	...
	*/
	...
	else
	{
		...
		if (strTalkID.length() == 4)	// 其他群里
		{
			isGroupTalk = true;
			// 获取 某个群，里面的 群员QQ号
			queryEmployeeModel.setQuery(QString("SELECT employeeID,picture FROM tab_employee WHERE status = 1 AND departmentID = %1")
				.arg(strTalkID));
		...
		{
			// 获取 对方QQ号
			queryEmployeeModel.setQuery(QString("SELECT picture FROM tab_employee WHERE status = 1 AND employeeID = %1")
				.arg(strTalkID));
			// 通过索引，找出图片路径，并转成 字符串
